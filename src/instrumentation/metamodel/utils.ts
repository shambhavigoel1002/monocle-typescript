...
function extractQueryFromContent(content) {
  try {
    ...
    console.warn(
      `Warning: Error occurred in extractQueryFromContent: ${e.toString()}`
    );
    return "";
  }
}
...
export function extractAssistantMessage(response) {
  try {
    ...
    console.warn(
      `Warning: Error occurred in extractAssistantMessage: ${e.toString()}`
    );
    return "";
  }
}
...
export function getLlmMetadata({ response, instance }) {
  ...
  if (response) {
    const tokenUsage =
      response.raw?.usage || response.response_metadata?.tokenUsage;
