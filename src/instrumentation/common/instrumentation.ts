...
export class MonocleInstrumentation extends InstrumentationBase {
  constructor(config = {}) {
  ...
  init() {
    ...
    const packagesForInstrumentation = combinedPackages.concat([]);
...
export function addSpanProcessors(okahuProcessors: any[] = []) {
  if (
    Object.prototype.hasOwnProperty.call(
      process.env,
      AWS_CONSTANTS.AWS_LAMBDA_FUNCTION_NAME
    )
  ) {
    const {
      AWSS3SpanExporter
    } = require("../../exporters/aws/AWSS3SpanExporter");
    okahuProcessors.push(
      new PatchedBatchSpanProcessor(new AWSS3SpanExporter({}), {
        scheduledDelayMillis: 5
      })
    );
    okahuProcessors.push(
      new PatchedBatchSpanProcessor(new ConsoleSpanExporter(), {
        scheduledDelayMillis: 5
      })
    );
  } else {
    okahuProcessors.push(
      new PatchedBatchSpanProcessor(getMonocleExporter(), {
        scheduledDelayMillis: 5
      })
    );
  }
